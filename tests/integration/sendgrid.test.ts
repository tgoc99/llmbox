...
Test ID: special-chars-${Date.now()}`,
    ...
    // Assert
    assertEquals(error, null, `Email with special chars should send successfully: ${error?.message}`);
...
Deno.test({
  ...
  async fn() {
    ...
    assertEquals(outgoingEmail.inReplyTo, incomingEmail.messageId, 'InReplyTo should be original message ID');
    assert(
...
Deno.test({
  ...
  async fn() {
    ...
    assertEquals(outgoingEmail.subject, 'Re: Previous conversation', 'Should not duplicate Re: prefix');
    assert(
  ...
  async fn() {
    ...
    for (let i = 1; i <= emailCount; i++) {
      const testEmail = createTestEmail({
        ...
        body: `This is test email ${i} of ${emailCount}.\n\nBatch ID: batch-${testId}\nEmail ID: ${i}`,
    ...
    assertEquals(errors.length, 0, `All emails should send successfully. Errors: ${errors.map(e => e.message).join(', ')}`);
...
if (!hasCredentials()) {
  ...
  console.log('   - TEST_RECIPIENT_EMAIL=test_recipient@domain.com (optional, defaults to service email)');
  console.log('   Then run: deno test tests/integration/sendgrid.test.ts --allow-all --allow-env\n');
}
